...
`,
  },
  {
    ...
    description:
      "New bookings on tours departing in 2027 or later now fall due on the second-to-last Friday of each month instead of the last Friday. Existing bookings keep their schedules.",
    date: "2026-09-22",
    version: "1.4.0",
    categories: ["feature", "improvement"],
    content: `# Second-to-Last Friday Due Dates for 2027 Tours

## Summary

Instalments have always been anchored to the **last Friday of each month**. For tours departing on or after **Jan 1, 2027**, new bookings are now anchored to the **second-to-last Friday** instead — one week earlier — giving travellers and the team a buffer before month-end.

Nothing changes for tours departing in 2026 or for any booking that already exists.

---

## The rule

A booking uses the second-to-last-Friday anchor only when **both** are true:

| Gate | Condition |
|---|---|
| Tour date | On/after **Jan 1, 2027** |
| Reservation date | On/after the **policy date** — Sep 23, 2026 |

Everything else is unchanged: instalments must fall after the reservation date + 2 days and on/before **2 calendar months before departure**, spacing never drops below 14 days, and the P4 cap holds.

### Worked example

Reserved Oct 6, 2026 for a tour departing Apr 10, 2027 (deadline Feb 10, 2027):

| | P1 | P2 | P3 | P4 |
|---|---|---|---|---|
| Last Friday (before) | Oct 30 | Nov 27 | Dec 25 | Jan 29 |
| Second-to-last Friday (after) | **Oct 23** | **Nov 20** | **Dec 18** | **Jan 22** |

### Snap-back under the new anchor

Snap-back now falls back to the latest **second-to-last** Friday on or before the deadline, so every due date on a 2027 schedule is a second-to-last Friday. Because that fallback is always one of the monthly anchors already in the schedule, snap-back no longer adds a term for these bookings — the anchor sitting a week earlier already provides the buffer snap-back existed to recover.

---

## What does *not* change

- **Existing bookings never move.** The reservation-date gate means a booking made before the policy date keeps its last-Friday schedule — even when its dates are regenerated by plan selection on the booking-status page, an admin recomputing a due-date column, or the snap-back backfill script.
- **2026 tours** keep the last-Friday anchor regardless of when they are reserved.
- **Legacy bookings** (reserved before Jun 1, 2026) are untouched.
- **Last Minute Bookings stay last-minute**, and the 2-month deadline, 14-day floor and 4-term cap all still apply.
- Stored field and column names (\`eligible2ndofmonths\`, "Eligible Last Fridays") are unchanged.

---

## Admin

- The **Schedule Policy** badge on a booking now reads "Standard schedule (2nd-to-last Friday)" when the new anchor applies.
- \`npm run report:2027-bookings -- --prod\` (read-only) counts 2027+ bookings by status/plan and lists any that would switch anchor under the gate — expected to be none.
...
| File | Change |
|---|---|
| \`lib/installment-schedule.ts\` | Second-to-last-Friday anchor + gate (\`usesSecondToLastFriday\`), matching snap-back fallback |
| \`lib/schedule-policy.ts\` | Optional \`tourDate\`; policy now carries an \`anchor\` and a distinct label |
| \`components/bookings/SchedulePolicyBadge.tsx\`, \`BookingDetailModal.tsx\`, \`LateFeesSection.tsx\` | Pass the tour date so the badge shows the right anchor |
| \`types/payment-terms.ts\` | Plan descriptions no longer hard-code "last Friday" |
| \`lib/installment-schedule.test.ts\`, \`bookingFlow.test.ts\` | 8 new tests incl. gate boundaries and an invariant sweep |
| \`scripts/report-2027-bookings.ts\` | **New.** Read-only 2027+ booking report |
`,
  },
  {
